d3.layout.orbit = function() {
	...
	_orbitLayout.speed = function(_degrees) {
		if (!arguments.length) return tickRadianStep / (Math.PI / 360 /3);
		tickRadianStep = tickRadianStep = _degrees * (Math.PI / 360 / 3);
		return this;
	}
			...
			if(childrenAccessor(_node)) {
				...
				if (fixedOrbitArray[_currentRing-1]) {
					...
					if (_node.parent) {
						...
						var _ring = {source: _node, x: _node.x, y: _node.y, r: (orbitSize[0] / 4) * (_currentRing / _rings)+150};
					}
